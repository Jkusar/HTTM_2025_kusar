...
def filter_names(names, exclude=None, include=None):
    """
    ...
    for name in names:
      ...
      if not any(sub in name for sub in exclude_c):
          # Check if the name contains all the substrings in include_name list
          if all(substring in os.path.basename(name) for substring in include):
...
def getFiles_recursive(folder_path, suffix, include=None, exclude=None, search_recursive=True):
    """
    ...
    Input parameters:
        ...
        - suffix [string] - The suffix (file extension) to match.
    ...
    if search_recursive:
        ...
        if suffix == None:
            pattern = os.path.join(folder_path, r"**/*")
    else:
        pattern = os.path.join(folder_path, f"*.{suffix}")
        if suffix == None:
            pattern = os.path.join(folder_path, r"**/*")

    files = glob.glob(pattern, recursive=search_recursive)

    filtered_files = filter_names(files, exclude=exclude, include=include)
    
    if not filtered_files:
        print("No files found. Try changing input parameters.")
...
def getFolders_recursive(directory_path, include=None, exclude=None, search_recursive=True):
    """
    Recursively finds files with the specified suffix in the given directory and its subdirectories.
...
def get_pressure_data(folder_path, l_conv, g_conv, p_conv):
    ...
    if os.path.isdir(folder_path):
        files = glob.glob(f"{folder_path}\**.txt", recursive=True)
    else:
        files = [folder_path]
...
def extract_frames(video_path, output_folder, frame_format="jpg", one_folder=False, error_path = None, report_existing_frames = False, limit_n_frames = None):
    """
    ...
    Input parameters:
        ...
        - output_folder [string] - path to the desired output folder. Folders with frames are saved in this folder. Folders are named after the videos.
...
def rename_folders(directory, old_substring, new_substring, report_missing = False, report_succes = False):
    """
    Renames folders in specified directory. Name changing is recursive, so folders that are saved 
    ...
    """
    folders = getFolders_recursive(directory_path=directory)   
...
def rename_files(directory, old_substring, new_substring, suffix, report_missing = False, report_succes = False):
    """
    ...
    if os.path.isdir(directory) is True:
        files = getFiles_recursive(folder_path=directory, suffix=suffix)   
    else:
        files = [directory]
    for file in files:
        try:
    ...
    # Sort the values in lists based on a separate list of index changes (if provided)
        ...
        for key, index_changes in zip(sorted_dict.keys(), sort_by_index):
            if isinstance(sorted_dict[key], list):
                ...
                reordered_list = original_list.copy()
...
def threshold_white(frame, filter_corner, filter_range):
    ...
    Input parameters:
        ...
        - filter_offset [int] - value of the ofset, by which white pixels are determined
    Output parameters:
...
def convertToBinary_thin_jets(frame, filter_threshold=255, filter_offset=0):
    """
...
def convert_multiple_frames_to_binary(directory, suffix, recursive_search=False, include=None, exclude=None):
    ...
    for i in all_frames_paths:
        img = cv2.imread(i)
        b_frame = convertToBinary_otsu(img)
...
def overlay_image(background, overlay, over_pos, over_resize, save_path, save_name="overlay.png", show_image=False):
    """
    Overlay specified images.
    
    Input parameters:
        - background [str]: path to the directory where background image is.
        - overlay [str]: path to the directory where overlay image is.
        - over_pos [list]: x and y position of the overlayed image on the background image
        - over_resize [list]: width and length of the overlayed image
        - save_path [str]: path to the saved image
        - save_name [str, optional]: name of the saved image
        - show_image[bool, optional]: option to show the final image
    """
    ...
    if over_resize is not None:
        overlay = overlay.resize(over_resize)  # width, height
    
    # Position (x, y) from top-left of background
    position = over_pos
    
    # Composite images
    background.paste(overlay, position, overlay)  # third arg is mask for transparency
    
    # Save or show result
    background.save(save_path + f"\{save_name}")
    if show_image:
        background.show()
        
def create_num_img(num, img_size, number_size, save_path, number_color = "black", number_font="arial.ttf",  bcg_color = "white", return_img = False):
    """
    Create an image of a specified number.
    
    Input parameters:
        - num [str]: number to create
        - img_size [list]: x and y size of the created image
        - number_size [int]: size of the number
        - save_path [str]: path to the saved image
        - number_color [str, optional]: color of the number
        - number_font [str, optional]: font of the created number 
        - bcg_color [str, optional]: color of the background
        - return_img [bool, optional]: option to return the image in array format.
    """
       
    # Create a blank white image
    img = Image.new('RGB', img_size, color=bcg_color)
    
    # Load a font
    font = ImageFont.truetype(number_font, number_size)  # You may need to specify the full path
    ...
    draw.text(position, number, fill=number_color, font=font)
